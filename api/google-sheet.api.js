...
export async function sheetUpdater(dataObject) {
    ...
    await sheet.loadCells(`A${rowCoutner}:AC${rowCoutner}`); //load the necessary row
    ...
    // Дата заказа
    const dateCell = sheet.getCellByA1(`E${rowCoutner}`);
    ...
    // ФИО
    const fioCell = sheet.getCellByA1(`F${rowCoutner}`);
    fioCell.value = dataObject.user;

    // Контакты
    const contactsCell = sheet.getCellByA1(`G${rowCoutner}`);
    contactsCell.value = new String(`@${dataObject.username}\n${dataObject.number}`); //now we have user's numbers!!

    // Адрес
    const destinationCell = sheet.getCellByA1(`H${rowCoutner}`);
    destinationCell.value = dataObject.destination;

    // DB ID пользователя
    const userDbIdCell = sheet.getCellByA1(`I${rowCoutner}`);
    userDbIdCell.value = dataObject.userId;

    // ORDER DB ID
    const orderDbIdCell = sheet.getCellByA1(`J${rowCoutner}`);
    orderDbIdCell.value = dataObject.orderId;

    // Корзина
    const cartCell = sheet.getCellByA1(`K${rowCoutner}`);
    let cartText = "";
    ...
    // Общий вес корзины
    const weightCell = sheet.getCellByA1(`L${rowCoutner}`);
    let cartWeight = 0;
    ...
    const chinaMoscowCell = sheet.getCellByA1(`N${rowCoutner}`);
    ...
    const moscowUserCell = sheet.getCellByA1(`P${rowCoutner}`);
    moscowUserCell.value = pricingConfig.rubDeliverySDEK;

    // Объявленная стоимость товара + наших услуг
    const totalPriceCell = sheet.getCellByA1(`R${rowCoutner}`);
    ...
    const cnyPriceCell = sheet.getCellByA1(`S${rowCoutner}`);
    let cnyPriceSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    const rubPriceCell = sheet.getCellByA1(`T${rowCoutner}`);
    let rubPriceSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    const feeErrorCell = sheet.getCellByA1(`U${rowCoutner}`);
    let feeErrorSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    // Комиссия сервиса WM
    const wmFeeCell = sheet.getCellByA1(`W${rowCoutner}`);
    let wmFeeSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    // Общая сумма пошлины
    const dutyFeeCell = sheet.getCellByA1(`Y${rowCoutner}`);
    let dutyFeeSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    // Наша прибыль
    const ourProfitCell = sheet.getCellByA1(`AC${rowCoutner}`);
    let ourProfitSum = 0;
    dataObject.cart.forEach((cartItem) => {
    ...
    const priceWithoutFeeCell = sheet.getCellByA1(`AA${rowCoutner}`);
...
export async function statusCellsGetter(rowNumber) {
    ...
    await sheet.loadCells(`A${rowNumber}:J${rowNumber}`) //1000 - произовльное большое значение

    const statusValue = sheet.getCellByA1(`C${rowNumber}`).value
    const sdekTrackNum = sheet.getCellByA1(`D${rowNumber}`).value
    const userDbIdValue = sheet.getCellByA1(`I${rowNumber}`).value
    const orderDbIdValue = sheet.getCellByA1(`J${rowNumber}`).value
